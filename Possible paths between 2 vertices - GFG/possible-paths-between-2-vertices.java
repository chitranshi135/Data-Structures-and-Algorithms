...
class Solution {
    ...
    public void helper(int source, int destination, ArrayList<ArrayList<Integer>> adj, int[] count) {
        if(source == destination){
            count[0]++;
            return;
        }
        for(int x: adj.get(source)){
            helper(x, destination, adj, count);
        }
    }
    ...
    public int countPaths(int V, ArrayList<ArrayList<Integer>> adj, int source,
                   int destination) {
        int[] count = {0};
        helper(source, destination, adj, count);
        return count[0];
    }
}